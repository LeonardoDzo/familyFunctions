...
export const onReadChat = functions.database.ref('/groups/{groupId}/members/{memberId}')
    .onUpdate(async snap => {
        ...
        let readsSnap = await admin.database().ref(`/groups/${groupId}/members`).once('value');
        let reads = readsSnap.val();
        let fartherRead = _.min(_.values(reads));
        console.log('farther', fartherRead);
        if (typeof fartherRead != 'number') return console.log('vv');
        let messagesSnap = await admin.database().ref('/messages')
